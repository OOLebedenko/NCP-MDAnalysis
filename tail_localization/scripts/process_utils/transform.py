...
class fit_rot_trans_by_pattern(fit_rot_trans):
    ...
    def __init__(self, ag, reference, pattern='all', *args, **kwargs):
        super().__init__(ag.select_atoms(pattern),
                         reference.select_atoms(pattern),
                         *args,
                         **kwargs)


class HistonePseudosymmetryAlign(TransformationBase):
    def __init__(self,
                 ag,
                 reference,
                 rotated_reference,
        ...
        self.fit_rot_trans_ref = fit_rot_trans(ag.select_atoms(pattern),
                                               reference.select_atoms(pattern),
                                               *args,
                                               **kwargs)
        self.fit_rot_trans_ref_rotated = fit_rot_trans(ag.select_atoms(pattern),
                                                       rotated_reference.select_atoms(pattern),
                                                       *args,
                                                       **kwargs)
        self.histone_ix = ag.select_atoms(f"chainID {histone_chain}").ix

    def _transform(self, ts):
        ts = self.fit_rot_trans_ref._transform(ts)
        ts_ref_rotated = self.fit_rot_trans_ref_rotated._transform(ts.copy())
        ts.positions[self.histone_ix] = ts_ref_rotated[self.histone_ix]
